...
CI および data-scp-docs ミラーでは、生成先の慣例として **`list/jp/jp_tag.json`** に書き出す（`-o list/jp/jp_tag.json`）。

data-scp-docs の `docs/catalog/scp_jp.json` 等へマージする際は、スラッグ正規化（-jp 付き）を
既存パイプラインに合わせて調整すること。
"""
...
def fetch(url: str) -> str:
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "scp-docs-tag-harvester/1.0 (+https://github.com/Kzky-Works/scp_docs)"},
    )
