...
正本の CI（data-scp-docs）とローカルでは、生成先の慣例として **`lists/jp/jp_tag.json`** に書き出す（`-o lists/jp/jp_tag.json`）。
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="write JSON file instead of stdout (e.g. lists/jp/jp_tag.json)",
    )
    args = ap.parse_args()
    data = harvest(args)
